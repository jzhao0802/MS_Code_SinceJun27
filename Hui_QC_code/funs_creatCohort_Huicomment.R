#Hui comments on 6/29/2016
#Hui: These are the functions using in createCohort.R
#     the main objective is to identify variable type and clean them

#function 1: identify variable type
varClassify <- function(dt){
  levels_cnt <- sapply(dt, function(x)length(na.omit(unique(x))))
  varLst <- names(dt)
  naVars <- varLst[levels_cnt ==0]#2 all NA
  cansVars <- varLst[levels_cnt==1] #41 constant
  biVars <- varLst[levels_cnt==2]   #277 binary
  #   catVars <- varLst[levels <= 10 & levels >2]
  #   contVars <- varLst[levels > 10]
  catVars <- varLst[levels_cnt <= 20 & levels_cnt > 2]#84 cat
  contVars <- varLst[levels_cnt > 20] #9 continuous
  # QC
...
#function 2 : combine small levels for catigorical variables
...
#function 3: combine small levels fro catigorical variables by order
...
#function 4: identify variable type, binary/catigorical/numeric
###################################################################################
#Hui: it seems you are using the default class when reading in or creating variable to identify
#     the type of variable, i am not saying it wrong, but a little bit dangerous to me. what if R
#     treat a binary as factor? Just suspect...
#     would it be a better idea to identify the type of variable as a prior-knowledge rather than 
#     detect it from data.
#####################################################################################
...
#function 5: convert catigorical to binary variables
